...
@section('content')
    <form action="{{route('admin.trainings.update', $training)}}" method="POST" id="addTrainingForm">
        @csrf
        @method('PUT')
        <div class="row">
            <div class="col-12 col-md-10 offset-md-1 border rounded py-3 px-2">
                <div class="col-12">
                    <div class="form-group">
                        ...
                        <input id="name" type="text" value="{{$training->name}}" name="name" class="form-control" placeholder="Nombre del plan de entrenamiento">
                    </div>
                </div>
                <div class="col-12">
                    <div class="form-group">
                        ...
                        <textarea id="description" name="description" class="form-control">{!! $training->description !!}</textarea>
                    </div>
                </div>
                        ...
                        <div class="row">
                            @foreach($routines as $r)
                                <div class="col-3">
                                    <label class="card">
                                        @if(in_array($r->id, $ro))
                                            <input class="card__input" type="checkbox" name="trainingRoutines[]" value="{{$r->id}}" checked/>
                                        @else
